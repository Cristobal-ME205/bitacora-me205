...
# 3. CONEXIÓN (Refrescamos la conexión para evitar el error de guardado)
conn = st.connection("gsheets", type=GSheetsConnection)

# Intentamos leer los datos; si falla, mostramos aviso
try:
    df_existente = conn.read(ttl=0)
except Exception as e:
    st.error("Error al conectar con el Excel. Verifica los permisos en Streamlit Cloud.")
...
with st.container(border=True):
    ...
    tipo_dia = st.selectbox("Clasificación de la Jornada", opciones)
...
# 5. LÓGICA DE GUARDADO (Corregida para evitar UnsupportedOperationError)
if btn_guardar:
    try:
        ...
        st.success(f"✅ Guardado correctamente: {tipo_final}")
        st.balloons()
        st.rerun()
    except Exception as e:
        st.error(f"No se pudo guardar. Revisa que el Excel no esté bloqueado o sin permisos.")

st.divider()

# 6. CONSULTA DE CÓMPUTO
